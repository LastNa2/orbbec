...
class Win32MouseController:
    """Enhanced mouse controller using win32api for better performance and reliability"""
    
    def __init__(self):
        self.last_click_time = 0
        self.min_click_interval = 0.1  # Minimum time between clicks (100ms)
        self.click_history = deque(maxlen=10)  # Track recent clicks for debugging
    
    def click(self, x, y, button='left', double_click=False):
        """
        Perform mouse click using win32api
        ...
        try:
            ...
            print(f"Win32 click error at ({x}, {y}): {e}")
            return False
    
    def get_click_statistics(self):
        """Get statistics about recent clicks"""
        if not self.click_history:
            return "No recent clicks"
        
        recent_clicks = len(self.click_history)
        time_span = self.click_history[-1]['time'] - self.click_history[0]['time'] if recent_clicks > 1 else 0
        click_rate = recent_clicks / max(time_span, 1)
        
        return f"Recent clicks: {recent_clicks}, Rate: {click_rate:.1f}/sec"
...
class ScreenFieldControlApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Multi-Person Skeleton Tracking with Win32 Mouse Control")
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)
        
        # Initialize Win32 mouse controller
        self.mouse_controller = Win32MouseController()
    ...
    def _create_ui(self):
        ...
        # Status label
        self.status_label = tk.Label(control_frame, text="Enhanced skeleton tracking with Win32 mouse ready", font=("Arial", 10))
        ...
        # Mouse statistics display
        self.mouse_stats_label = tk.Label(control_frame, text="Mouse Stats: No clicks yet", font=("Arial", 9))
        self.mouse_stats_label.pack(pady=2)
    ...
    def capture_loop(self):
        ...
        try:
            while self.capturing:
                ...
                self.mouse_stats_label.config(text=f"Mouse Stats: {self.mouse_controller.get_click_statistics()}")
    ...
    def camera_mapping_loop(self):
        """Main camera mapping loop with enhanced skeleton tracking - FIXED VERSION"""
        dev = None
        depth_stream = None
        color_stream = None
        ...
        try:
            # Initialize OpenNI2
            print(f"Initializing OpenNI2 from path: {Config.OPENNI_PATH}")
            try:
                openni2.initialize(Config.OPENNI_PATH)
            except Exception as e:
                print(f"Failed to initialize with custom path: {e}")
            ...
            try:
                openni2.unload()
                print("OpenNI2 unloaded successfully")
            except Exception as unload_error:
                print(f"OpenNI2 unload error: {unload_error}")
            
            self.mapping_active = False
            print("Camera mapping loop ended")
    ...
    def _handle_enhanced_square_interaction(self, color_image, person_id, hand_name, square_idx,
                                          pixel_x, pixel_y, depth_val, mapped_pos):
        """Handle enhanced square interaction with improved accuracy using Win32"""
        person_state = self.persons[person_id]
        ...
        # Enhanced click handling with stability requirement using Win32
        if should_click and stability > self.tracking_quality_threshold:
            # Use Win32 mouse controller instead of pyautogui
            click_success = self.mouse_controller.click(screen_x, screen_y, button='left')
            ...
            if click_success:
                ...
                print(f"Win32 click: Person {person_id} {hand_name} at ({screen_x}, {screen_y}) "
                ...
                # Add Win32-specific feedback
                cv2.putText(color_image, "WIN32 CLICK", 
                           (pixel_x + 15, pixel_y + 55), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
            else:
                # Click failed - show warning
                cv2.putText(color_image, "CLICK FAILED", 
                           (pixel_x + 15, pixel_y + 55), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
    ...
    def _draw_enhanced_status_info(self, color_image):
        ...
        cv2.putText(color_image, f"Enhanced Tracking (Win32) - Persons: {person_count}, Touches: {total_touches}", 
        ...
        # Win32 mouse statistics
        mouse_stats = self.mouse_controller.get_click_statistics()
        cv2.putText(color_image, f"Win32 Mouse: {mouse_stats}", 
                   (20, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
        
        # Enhanced help text
        help_texts = [
            "Enhanced Skeleton Tracking with Win32 Controls:",
    ...
    def _draw_grid(self, image, square_data):
        ...
        for i in range(self.grid_resolution + 1):
            start_idx = i * (self.grid_resolution + 1)
            end_idx = (i + 1) * (self.grid_resolution + 1)
            row_points = grid_points[start_idx:end_idx]
            
            if start_idx < len(square_data["grid_points_3d"]):
                row_depths = [square_data["grid_points_3d"][start_idx + j][2] 
                             for j in range(min(self.grid_resolution + 1, len(square_data["grid_points_3d"]) - start_idx))]
                
                for j in range(len(row_points) - 1):
                    if j < len(row_depths) - 1:
        ...
        # Draw vertical lines
        for j in range(self.grid_resolution + 1):
            col_indices = [i * (self.grid_resolution + 1) + j for i in range(self.grid_resolution + 1)]
            col_points = [grid_points[idx] for idx in col_indices if idx < len(grid_points)]
            col_depths = [square_data["grid_points_3d"][idx][2] for idx in col_indices if idx < len(square_data["grid_points_3d"])]
            
            for i in range(len(col_points) - 1):
                if i < len(col_depths) - 1:
                    pt1 = tuple(col_points[i])
                    pt2 = tuple(col_points[i + 1])
    ...
    def stop_all(self):
        ...
        self.status_label.config(text="Enhanced tracking with Win32 stopped")
...
def test_win32_mouse():
    """Test function for Win32 mouse functionality"""
    print("Testing Win32 mouse controller...")
    ...
    print("Moving cursor to (100, 100)...")
    controller.move_cursor(100, 100)
    time.sleep(1)
    
    # Test click
    print("Performing test click...")
    success = controller.click(100, 100)
    print(f"Click successful: {success}")
    
    # Test drag
    print("Performing test drag...")
    success = controller.drag(100, 100, 200, 200, duration=1.0)
    print(f"Drag successful: {success}")
    
    # Test scroll
    print("Performing test scroll...")
    success = controller.scroll(150, 150, clicks=3, direction='up')
    print(f"Scroll successful: {success}")
    
    # Get statistics
    stats = controller.get_click_statistics()
    print(f"Click statistics: {stats}")
    
    print("Win32 mouse controller test completed!")


def verify_openni_installation():
    """Verify OpenNI2 installation and provide troubleshooting tips"""
    print("Verifying OpenNI2 installation...")
    
    # Check if OpenNI2 path exists
    openni_path = Config.OPENNI_PATH
    if os.path.exists(openni_path):
        print(f"[OK] OpenNI2 path exists: {openni_path}")
        ...
        for file in essential_files:
            file_path = os.path.join(openni_path, file)
            if os.path.exists(file_path):
                found_files.append(file)
                print(f"[OK] Found: {file}")
        
        if not found_files:
            print("[ERROR] No essential OpenNI2 files found in the specified directory")
            print("Please verify your OpenNI2 installation")
    else:
        print(f"[ERROR] OpenNI2 path does not exist: {openni_path}")
        print("Please update the OPENNI_PATH in Config class")
    
    # Test OpenNI2 import
    try:
        from openni import openni2
        print("[OK] OpenNI2 Python module imported successfully")
    except ImportError as e:
        print(f"[ERROR] Failed to import OpenNI2 module: {e}")
        print("Please install primesense OpenNI2 Python bindings")
        return False
    ...
    try:
        ...
        print(f"[INFO] Found {len(devices)} OpenNI2 devices")
        
        if len(devices) == 0:
            print("[WARNING] No OpenNI2 devices found!")
            print("Troubleshooting tips:")
            print("1. Ensure your depth camera is connected")
            print("2. Check USB drivers are installed")
            print("3. Try running as administrator")
            print("4. Verify camera is not being used by another application")
            print("5. Try unplugging and reconnecting the camera")
        else:
            for i, device_uri in enumerate(devices):
                print(f"  Device {i}: {device_uri}")
        
        openni2.unload()
        print("[OK] OpenNI2 unloaded successfully")
        return len(devices) > 0
        
    except Exception as e:
        print(f"[ERROR] OpenNI2 initialization failed: {e}")
        print("Troubleshooting tips:")
        print("1. Ensure your depth camera is connected")
        print("2. Check USB drivers are installed")
        print("3. Try running as administrator")
        print("4. Verify OpenNI2 path is correct")
        print("5. Make sure camera is not in use by another application")
        return False
...
def main():
    ...
    try:
        print("Enhanced Multi-Person Skeleton Tracking with Win32 Mouse Control")
        print("=" * 60)
        ...
        # Verify OpenNI2 installation
        openni_ok = verify_openni_installation()
        print()
        
        if not openni_ok:
            print("WARNING: No depth camera detected!")
            print("The application may not work properly without a depth camera.")
            response = input("Continue anyway? (y/n): ")
            if response.lower() != 'y':
                return
        
        # Uncomment the line below to test Win32 mouse functionality
        # test_win32_mouse()
        ...
        print("Application GUI created successfully")
        print("Instructions:")
        print("1. First select screen regions using 'Choose Screen Field' buttons")
        print("2. Then use '3D Square Mapping' to define tracking areas")
        print("3. Use 'Start Enhanced Tracking' to begin the application")
        print("4. Make sure your depth camera is connected and working")
        ...
        print("2. Check OpenNI2 path configuration")
        print("3. Verify depth camera is connected")
        print("4. Try running as administrator")
        print("5. Make sure no other applications are using the camera")
        input("Press Enter to exit...")
...
# working, just need to test in the cube and with unity full screen exclusive
